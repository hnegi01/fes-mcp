...
# Captured result shapes, generated by scripts/06_capture_output_schemas.py
# from live runs (the SDK declares no return contracts). Lives next to the
# registry file; absent file means tools simply advertise no outputSchema.
OUTPUT_SCHEMAS_FILENAME = "output_schemas.json"


def _load_output_schemas(registry_path: Path) -> dict[str, Any]:
    path = registry_path.parent / OUTPUT_SCHEMAS_FILENAME
    if not path.exists():
        return {}
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        logger.warning("Ignoring unreadable output schemas at %s: %s", path, exc)
        return {}
...
def load_registry(path: Path) -> dict[str, dict[str, Any]]:
    ...
    output_schemas = _load_output_schemas(path)
    for tool_id, schema in output_schemas.items():
        if tool_id in tools:
            tools[tool_id]["output_schema"] = schema
        else:
            logger.warning(
                "output_schemas.json names unknown tool %s (renamed by a registry "
                "refresh?) — re-run scripts/06_capture_output_schemas.py",
                tool_id,
            )
